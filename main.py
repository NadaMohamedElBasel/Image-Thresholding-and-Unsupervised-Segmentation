from PyQt5.QtWidgets import QApplication,QMessageBox, QSpinBox, QDoubleSpinBox, QWidget, QVBoxLayout, QHBoxLayout, QLabel, QPushButton, QSlider, QGroupBox, QGridLayout, QGraphicsView, QFileDialog, QGraphicsScene, QGraphicsPixmapItem
...
class ClickableGraphicsView(QGraphicsView):
    ...
    def mousePressEvent(self, event):
        if event.button() == Qt.LeftButton:
            ...
            self.clicked.emit(int(pos.y()), int(pos.x()))  # row = y, col = x
...
class CVApp(QWidget):
    ...
    def initUI(self):
        ...
        self.input_view = ClickableGraphicsView()
        self.input_view.clicked.connect(self.handle_image_click)
...
#################################################################################################################################################
    ...
    def region_growing(self):
        if not hasattr(self, 'seed_point'):
            QMessageBox.warning(self, "Warning", "Please click on the image to select a seed point.")
            return
        ...
        segmented_image_rg = cv2.bitwise_and(image, image, mask=mask)
        gray_image = cv2.cvtColor(segmented_image_rg, cv2.COLOR_BGR2GRAY)
